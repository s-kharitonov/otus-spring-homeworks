...
public class MongoChangeLog {
	...
	public void initAuthors(final AuthorsRepository authorsRepository) {
		...
		authorsByName.put(KAREN, authorsRepository.save(karen));
		authorsByName.put(ROBERT, authorsRepository.save(robert));
		authorsByName.put(MAXIM, authorsRepository.save(maxim));
		authorsByName.put(NICHOLAS, authorsRepository.save(nikolas));
	}
	...
	public void initGenres(final GenresRepository genresRepository) {
		...
		genresByName.put(FANTASY, genresRepository.save(fantasy));
		genresByName.put(PROGRAMMING_LANGUAGES, genresRepository.save(programs));
		genresByName.put(LITERATURE, genresRepository.save(literature));
		genresByName.put(ROMANCE, genresRepository.save(romance));
	}
	...
	public void initBooks(final BooksRepository booksRepository) {
		...
		booksByName.put(ARCHITECTS_OF_MEMORY, booksRepository.save(architects));
		booksByName.put(ENGINES_OF_OBLIVION, booksRepository.save(engines));
		booksByName.put(CLEAN_CODE_A_HANDBOOK_OF_AGILE_SOFTWARE_CRAFTSMANSHIP, booksRepository.save(cleanCode));
		booksByName.put(CLEAN_ARCHITECTURE_A_CRAFTSMAN_S_GUIDE_TO_SOFTWARE_STRUCTURE_AND_DESIGN, booksRepository.save(cleanArchitecture));
		booksByName.put(RUTHIE_FEAR_A_NOVEL, booksRepository.save(ruthieFear));
		booksByName.put(COME_WEST_AND_SEE_STORIES, booksRepository.save(comeWest));
		booksByName.put(THE_RETURN, booksRepository.save(theReturn));
		booksByName.put(THE_RESCUE, booksRepository.save(theRescue));
	}
	...
	public void initBookComments(final BookCommentsRepository bookCommentsRepository) {
		...
		bookCommentsRepository.save(perfectComment);
		bookCommentsRepository.save(amazingComment);
		bookCommentsRepository.save(interestingComment);
	}
}
